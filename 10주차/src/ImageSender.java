...
public class ImageSender {
	...
	public static int SLEEP_MILLIS = 1000;
	public static String IP_ADDRESS =  "230.0.0.1";
	...
	public static BufferedImage getScreenshot() throws AWTException,IIOException,IOException{
	 ...
	 * Multicast addresses are IP addresses in the range of 230.0.0.0 to
	 * 230.255.255.255.
	 */
	...
	private boolean sendImage(byte[] imageData,String multicastAddress,int port) {
		...
		try {
			...
			ms.send(dp);
			ret = true;
			System.out.println("sent captured image ^^....");
		}catch(IOException e) {
			e.printStackTrace();
			ret = false;
		}finally {
			...
			while(true) {
				...
				if(multicastImages) {
					...
					if(SHOW_MOUSEPOINTER) {
						...
						Polygon polygon1 = new Polygon(new int[] {mouseX,mouseX+10,mouseX,mouseX},new int[] {mouseY,mouseY+10,mouseY+15,mouseY},4);
						Polygon polygon2 = new Polygon(new int[] {mouseX+1,mouseX+10+1,mouseX+1,mouseX+1},new int[] {mouseY+1,mouseY+10+1,mouseY+15+1,mouseY+1},4);
